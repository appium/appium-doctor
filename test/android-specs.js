...
describe('android', function () {
  describe('EnvVarAndPathCheck', withMocks({fs}, (mocks) => {
    ...
    it('fix', async function () {
      (await check.fix()).should.equal(`Manually configure ${'ANDROID_HOME'.bold}.`);
    });
  }));
  describe('AndroidToolCheck', withMocks({fs}, (mocks) => {
    ...
    it('fix - install', async function () {
      process.env.ANDROID_HOME = '/a/b/c/d';
      (await check.fix()).should.equal(`Manually install ${'adb'.bold} and add it to ${'PATH'.bold}.`);
    });
  }));
  ...
  describe('OptionalAppBundleCheck', withMocks({tp, utils}, (mocks) => {
    ...
    it('fix', async function () {
      (await check.fix()).should.equal(`${'bundletool.jar'.bold} is used to handle Android App Bundle. Please read http://appium.io/docs/en/writing-running-appium/android/android-appbundle/ to install it`);
    });
  }));
});
