...
class OptionalGstreamerCheck extends DoctorCheck {
  ...
  BINARY_PAIR_MSG = `${this.GSTREAMER_BINARY} and ${this.GST_INSPECT_BINARY}`;
  ...
  async diagnose () {
    ...
    return gstreamerPath && gstInspectPath
      ? okOptional(`${this.BINARY_PAIR_MSG} are installed at: ${gstreamerPath} and ${gstInspectPath}`)
      : nokOptional(`${this.BINARY_PAIR_MSG} cannot be found`);
  }

  async fix () { // eslint-disable-line require-await
    return `${`${this.BINARY_PAIR_MSG}`.bold} are used to stream the screen of the device under test. ` +
