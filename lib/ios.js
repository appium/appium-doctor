...
class CarthageCheck extends DoctorCheck {
  async diagnose () {
    ...
    if (carthagePath) {
      let stdout = '';
      try {
        ({stdout} = await exec(carthagePath, ['version']));
        return ok(`Carthage was found at: ${carthagePath}. Installed version is: ${stdout.trim().split('\n').reverse()[0]}`);
      } catch (err) {
        log.warn(err);
      }
    }
    return nok(`Carthage was NOT found!`);
  }
