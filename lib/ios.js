...
class OptionalIdbCommandCheck extends DoctorCheck {
  async diagnose () {
    try {
      ...
      // Error code 2 happens if idb_companion has not been installed
      // while fb-idb has been installed
      await exec(fbIdbPath);
      if (fbIdbPath) {
        return okOptional('idb are installed');
      }
    } catch {}
  ...
  async fix () { // eslint-disable-line require-await
    return 'Why idb is needed and how to install it: https://github.com/appium/appium-idb';
  }
}
...
export {
  ...
  AuthorizationDbCheck, CarthageCheck, OptionalIdbCommandCheck, OptionalApplesimutilsCommandCheck, OptionalIdevicelocationCommandCheck,
