...
import { EOL } from 'os';
import { isWindows } from 'appium-support/build/lib/system';
...
async function getNpmCmdPath () {
  return isWindows()
    ? await resolveExecutablePath('npm.cmd')
    : await resolveExecutablePath('npm');
}
...
class OptionalOpencv4nodejsCommandCheck extends DoctorCheck {
  ...
  async diagnose () {
    ...
    const npmPath = await getNpmCmdPath();

    if (!npmPath) {
      log.debug(`'npm' binary not found in PATH: ${process.env.PATH}`);
      return nokOptional(`'npm' binary cannot be found.`);
    }
