...
class PythonVersionCheck extends DoctorCheck {
  async diagnose () {
    ...
    if (Number.isNaN(version)) {
      return nok(`Unable to identify Python version correctly (version = '${versionString}') at ${pythonPath}. Please make sure your Python environment in PATH: ${process.env.PATH}. node-gyp requires Python 2.x`);
    }
