...
class OptionalOpencv4nodejsCommandCheck extends DoctorCheck {
  async diagnose () {
    ...
    let pJson;
    try {
      ({stdout} = await exec(npmPath, ['list', '-g', '-j', packageName]));
    ...
    if (pJson.dependencies && pJson.dependencies[packageName]) {
      ...
      return okOptional(`${packageName} is installed at: ${lines[0]}. Installed version is: ${pJson.dependencies[packageName].version}`);
    }
